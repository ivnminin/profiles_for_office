...
class OrderComputerForm(FlaskForm):
    ...
    description = TextAreaField("Description", validators=[DataRequired(), Length(min=1, max=2048)],
...
class ConsultationForm(FlaskForm):
    ...
    organization = StringField("Organization", validators=[DataRequired(), Length(min=1, max=255)])
    submit = SubmitField("Submit")


class GroupOrderForm(FlaskForm):
    title = StringField("Title", validators=[DataRequired(), Length(min=1, max=255)], render_kw={"autocomplete": "off"})
